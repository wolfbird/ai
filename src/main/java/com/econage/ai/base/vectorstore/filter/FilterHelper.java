...
package com.econage.ai.base.vectorstore.filter;

import com.econage.ai.base.vectorstore.filter.Filter;
import com.econage.ai.base.vectorstore.filter.Filter.Expression;
import com.econage.ai.base.vectorstore.filter.Filter.ExpressionType;
import com.econage.ai.base.vectorstore.filter.Filter.Operand;
...
public final class FilterHelper {
		...
		if (operand instanceof Filter.Group group) {
			...
			switch (exp.type()) {
				...
				case LTE: // NOT(e LTE b) = e GT b
					return new Expression(TYPE_NEGATION_MAP.get(exp.type()), exp.left(), exp.right());
	...
	private static void expandInNinExpressions(ExpressionType outerExpressionType,
                                               ExpressionType innerExpressionType, Expression exp, StringBuilder context,
		...
		if (exp.right() instanceof Filter.Value value) {
			if (value.value() instanceof List list) {
				...
				// "bar3")))
				List<Expression> eqExprs = new ArrayList<>();
				for (Object o : list) {
					eqExprs.add(new Expression(innerExpressionType, exp.left(), new Filter.Value(o)));
				}
				...
				context.append(expressionConverter
					.convertExpression(new Expression(innerExpressionType, exp.left(), exp.right())));
			}
		}
		else {
	 ...
	 */
	private static Expression aggregate(ExpressionType aggregateType,
                                        List<Expression> expressions) {

		if (expressions.size() == 1) {
			return expressions.get(0);
		}
		return new Expression(aggregateType, expressions.get(0),
