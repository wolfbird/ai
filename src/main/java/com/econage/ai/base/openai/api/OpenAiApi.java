...
package com.econage.ai.base.openai.api;

import com.econage.ai.base.openai.api.common.OpenAiApiConstants;
import com.econage.ai.support.exception.ChatException;
import com.econage.ai.support.utils.JsonUtils;
import com.econage.ai.support.utils.StringMore;
...
public class OpenAiApi {
	...
	public OpenAiApi(String baseUrl, String apiKey, MultiValueMap<String, String> headers, String completionsPath,
		...
		this.restClient = restClientBuilder.baseUrl(baseUrl)
				...
				.requestFactory(ClientHttpRequestFactories.get(new ClientHttpRequestFactorySettings(Duration.ofSeconds(20),Duration.ofSeconds(60), (SslBundle) null)))
				.requestInterceptor((request, body, execution) -> {
					log.info("chat Request URL: {}", request.getURI());
					request.getHeaders().forEach((name, values) -> log.info("chat Request Header: {} = {}", name, values));
					return execution.execute(request, body);
				})
				.build();

		this.webClient = webClientBuilder
				.clientConnector(new JdkClientHttpConnector(HttpClient.newBuilder().version(apiKey!=null&&apiKey.startsWith("gpustack")?HttpClient.Version.HTTP_1_1:HttpClient.Version.HTTP_2).build()))
				.filter(logRequest())
				.filter(ExchangeFilterFunction.ofResponseProcessor(response ->{
					if (response.statusCode().isError()) {
						return response.bodyToMono(String.class).flatMap(body -> Mono.error(new ChatException(response.statusCode()+StringMore.SPACE+body)));
					}
	...
	private ExchangeFilterFunction logRequest() {
		return ExchangeFilterFunction.ofRequestProcessor(request -> {
			...
			log.info("chat Request URL: {}", request.url());
			request.headers().forEach((name, values) -> log.info("chat Request Header: {} = {}", name, values));
	...
	public ResponseEntity<ChatCompletion> chatCompletionEntity(ChatCompletionRequest chatRequest,
		...
		log.info("chat Request Body : {} " , JsonUtils.objectToJson(chatRequest));
